...
pod_spec = {
    "containers": [
        {"image": "foo", "name": "foo", "command": ["foo"], "resources": {}}
    ],
    "apiVersion": "v1",
    "kind": "Pod",
    "metadata": {"name": "foo"},
...
def test_set_default_resources4():
    os.environ["ADAPTDL_JOB_DEFAULT_RESOURCES"] = '{"limits": {"memory": 200}}'
    pod_spec_copy = copy.deepcopy(pod_spec)
    pod_spec_copy["containers"][0]["resources"] = {"limits": {"memory": 300}}
    pod_spec_copy = resources.set_default_resources(pod_spec_copy)
    res = pod_spec_copy["containers"][0]["resources"]
    ...
    "containers": [
        {
            ...
            "command": ["foo"],
            "resources": {
                "requests": {"memory": 100, "cpu": 1.5},
                "limits": {"memory": 80, "cpu": 1, "cows": 2},
            },
        },
        {
            "image": "foo2",
            ...
            "command": ["foo2"],
            "resources": {
                "requests": {"memory": 120, "cpu": 2.5},
                "limits": {"memory": 100, "cpu": 2, "cows": 5},
            },
        },
        {
            "image": "foo3",
            ...
            "command": ["foo3"],
            "resources": {
                "requests": {"memory": 250, "cpu": 10},
                "limits": {"memory": 250, "cpu": 10},
            },
        },
    ],
    "apiVersion": "v1",
