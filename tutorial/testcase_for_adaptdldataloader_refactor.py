...
from adaptdl.torch.data import AdaptiveDLContext # For test AdaptiveDLContext only, users do not need to call this
...
def train(args, model, device, train_loader, optimizer, epoch, adacontext): # For test AdaptiveDLContext only, users do not need to call this
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'
                ...
                adacontext.get_batch_size(), adacontext.get_accum_steps(), adacontext.get_lr_scale(),# For test AdaptiveDLContext only, users do not need to call this
            ))
            if args.dry_run:
                break
...
def main():
    ...
    dataset2 = datasets.MNIST('../data', train=False,
                       transform=transform)
    adacontext = AdaptiveDLContext(args.batch_size) # For test AdaptiveDLContext only, users do not need to call this
    ...
    for epoch in adaptdl.torch.remaining_epochs_until(args.epochs): # Changed in step 4
        train(args, model, device, train_loader, optimizer, epoch, adacontext) # For test AdaptiveDLContext only, users do not need to call this
