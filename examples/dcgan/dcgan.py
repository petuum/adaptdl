...
# Radford adl. al. in the paper `Unsupervised Representation Learning With
...
dataroot = adaptdl.env.share_path()
...
dataset = dsets.CelebA(dataroot, split='train', target_type='attr', transform=transform, target_transform=None, download=True)
...
# one for :math:`G`. While the DCGAN paper specifies Adam optimizers with
# learning rate 0.0002 and Beta1 = 0.5, we use SGD optimizers with learning rate
# 0.0005, momentum 0.9 and Nesterov momentum. Note: these setting are not well-tuned.
# For keeping track of the generator’s learning progression, we will generate a fixed batch
...
scheduleD = optim.lr_scheduler.StepLR(optimizerD, 15, 0.1)
scheduleG = optim.lr_scheduler.StepLR(optimizerG, 15, 0.1)

adaptdl.torch.init_process_group("nccl" if torch.cuda.is_available()
                                     else "gloo")
netD = adl.AdaptiveDataParallel(netD, optimizerD, scheduleD, "netD")
netG = adl.AdaptiveDataParallel(netG, optimizerG, scheduleG, "netG")
